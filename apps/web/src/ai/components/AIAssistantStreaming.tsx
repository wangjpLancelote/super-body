...
interface AIAssistantStreamingProps {
  enableStreaming?: boolean;
  dryRun?: boolean;
}

export function AIAssistantStreaming({
  enableStreaming = true,
  dryRun = true
}: AIAssistantStreamingProps) {
  const { user } = useAuth();
    ...
    try {
      if (accessToken) {
        if (enableStreaming) {
          ...
          setIsStreaming(true);
          const streamingResponse = AIService.chatStream({
            message: content,
            context: {
              userId: user?.id,
        ...
        if (content.includes('文档') || content.includes('搜索')) {
          const mockResponse = mockResponses.searchDocuments(content);
          const responseMessage: Message = {
            id: 'assistant-' + Date.now(),
            content: `我找到了 ${mockResponse.documents.length} 个相关文档：`,
            role: 'assistant',
            timestamp: new Date().toISOString(),
            type: 'search-results',
            data: mockResponse.documents,
          };
          setMessages(prev => [...prev, responseMessage]);
          ...
          const mockResponse = mockResponses.createTodo(title, content);
          const responseMessage: Message = {
            id: 'assistant-' + Date.now(),
            content: mockResponse.message || '任务创建成功！',
            role: 'assistant',
            timestamp: new Date().toISOString(),
            type: 'action',
          ...
          const responseMessage: Message = {
            id: 'assistant-' + Date.now(),
            content: `我理解你的问题："${content}"。让我帮你处理这个请求。`,
            role: 'assistant',
            timestamp: new Date().toISOString(),
            type: 'text',
          };
